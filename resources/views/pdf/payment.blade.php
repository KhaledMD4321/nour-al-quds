...
<body>
    ...
    <table class="info-table">
        <tr><td>رقم السند</td><td><span class="info-ref">{{ $payment->payment_number }}</span></td></tr>
        <tr><td>التاريخ</td><td>{{ $payment->payment_date?->format('d/m/Y') }}</td></tr>

        @if($isSupplier && $payment->supplier)
        <tr>
            <td>المورد</td>
            <td>{{ $payment->supplier->name }}
                @if($payment->supplier->code)
                    <span style="color:#9ca3af; font-size:10px; font-weight:400;">({{ $payment->supplier->code }})</span>
                @endif
            </td>
        </tr>
        @if($payment->supplier->phone)
        <tr><td>هاتف المورد</td><td>{{ $payment->supplier->phone }}</td></tr>
        @endif
        @endif

        @if($isExpense)
        <tr><td>تصنيف المصروف</td><td>{{ $payment->category_label }}</td></tr>
        @if($payment->expenseAccount)
        <tr><td>حساب المصروف</td><td>{{ $payment->expenseAccount->code }} — {{ $payment->expenseAccount->name }}</td></tr>
        @endif
        @endif

        @if($payment->purchaseInvoice)
        <tr><td>فاتورة الشراء</td><td>{{ $payment->purchaseInvoice->reference_number }}</td></tr>
        <tr>
        ...
        @if($payment->treasury)
        <tr><td>الخزينة</td><td>{{ $payment->treasury->name }}</td></tr>
        @endif
        <tr><td>الوحدة التشغيلية</td><td>{{ $payment->businessUnit?->name }}</td></tr>
        @if($payment->bank_reference)
        <tr><td>مرجع التحويل</td><td>{{ $payment->bank_reference }}</td></tr>
        @endif
        @if($payment->journalEntry)
        <tr><td>القيد المحاسبي</td><td>{{ $payment->journalEntry->entry_number }}</td></tr>
        @endif
        @if($payment->createdBy)
        <tr><td>أنشأه</td><td>{{ $payment->createdBy->name }}</td></tr>
        @endif
    </table>
    ...
    <div class="sig-wrap">
        <div class="sig-cell"><div class="sig-line">المُحرِّر</div></div>
        <div class="sig-cell"><div class="sig-line">المستلم</div></div>
    </div>

    <div class="footer">
        {{ $company?->name ?? \App\Models\SystemSetting::get('company.name', 'نور القدس') }}
        @if($company?->phone) | هاتف: {{ $company->phone }}@endif
        | {{ $payment->payment_number }} — {{ $payment->payment_date?->format('d/m/Y') }}
    </div>
