    ...
    <div dir="rtl" style="font-family: Cairo, sans-serif;">
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 20px;">
            @foreach([
                ...
                ['label' => 'إجمالي المبيعات',    'value' => number_format($summary->total_amount, 2),   'unit' => 'ج.م',  'color' => '#166534', 'bg' => '#f0fdf4'],
                ['label' => 'المحصّل',             'value' => number_format($summary->paid_amount, 2),    'unit' => 'ج.م',  'color' => '#059669', 'bg' => '#dcfce7'],
                ['label' => 'المتبقي',             'value' => number_format($summary->outstanding, 2),   'unit' => 'ج.م',  'color' => '#dc2626', 'bg' => '#fef2f2'],
            ] as $card)
                ...
                <thead style="background: #1e40af; color: white;">
                    <tr>
                        ...
                        @else
                            ...
                            <th style="padding: 10px 14px; text-align: left; font-size: 12px;">المحصّل</th>
                            <th style="padding: 10px 14px; text-align: left; font-size: 12px;">المتبقي</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                    @forelse($data as $row)
                        <tr style="border-bottom: 1px solid #f3f4f6;" onmouseover="this.style.background='#f9fafb'" onmouseout="this.style.background=''">
                            @if($isByProduct)
                                ...
                                <td style="padding: 8px 14px; text-align: left; font-size: 13px; font-weight: 600; color: #166534;">{{ number_format($row->total_revenue, 2) }}</td>
                            @else
                ...
                <tfoot style="background: #111827; color: white;">
                    <tr>
                        @if($isByProduct)
                            ...
                            <td style="padding: 12px 14px; text-align: left; font-weight: 700; color: #86efac;">{{ number_format($data->sum('total_revenue'), 2) }} ج.م</td>
                        @else
                            ...
                            <td style="padding: 12px 14px; text-align: left; font-weight: 700; color: #86efac;">{{ number_format($data->sum('total'), 2) }} ج.م</td>
