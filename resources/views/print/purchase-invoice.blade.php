...
@php
    ...
    $docLabel     = 'فاتورة مشتريات — ' . $invoice->reference_number;
@endphp

@section('title', $docLabel)
@section('toolbar-title', $docLabel . ($invoice->supplier ? ' | ' . $invoice->supplier->name : ''))

@section('styles')
<style>
    body { font-size: 11.5px; line-height: 1.5; color: #1f2937; }
    ...
    .company-name { font-size: 19px; font-weight: 800; color: {{ $primaryColor }}; margin-bottom: 4px; }
    ...
    .info-box {
        ...
        vertical-align: top; padding: 9px 11px;
    ...
    table.items {
        ...
        margin-bottom: 13px; font-size: 10.5px;
    }
    ...
    .notes-box {
        ...
        margin-bottom: 13px; font-size: 10px;
        color: #4b5563; line-height: 1.7;
    }
    .notes-title { font-weight: 700; color: #374151; margin-bottom: 2px; }
    ...
    .sig-line {
        ...
        font-size: 10.5px; font-weight: 700; margin-top: 36px;
    }
    ...
    <div class="info-section">
        ...
        <div class="info-box">
            ...
            <div class="info-row">
                ...
                <span style="font-weight:800; color:{{ $primaryColor }};">{{ $invoice->reference_number }}</span>
            </div>
            ...
            <div class="info-row">
                ...
                <span class="status-badge" style="background:{{ $invoice->status === 'paid' ? '#dcfce7' : ($invoice->status === 'confirmed' ? '#dbeafe' : '#f3f4f6') }}; color:{{ $invoice->status === 'paid' ? '#166534' : ($invoice->status === 'confirmed' ? '#1e40af' : '#4b5563') }};">
    ...
    <table class="items">
        ...
        <tbody>
            @foreach($invoice->items as $i => $item)
                <tr>
                    <td class="c">{{ $i + 1 }}</td>
                    <td><span style="font-weight:600;">{{ $item->product?->name ?? 'صنف محذوف' }}</span></td>
